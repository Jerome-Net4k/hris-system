...
if (isset($_POST['searchinput'])) {
    ...
    nosiloadsearch($salninputsearch, $startyear, $endyear);
} else {
    loadlist();
}

function nosiloadsearch($salninputsearch, $startyear, $endyear) {
    include "connection.php";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            for ($i = $startyear; $i > $endyear; $i--) {
                $stmt = $con->prepare("SELECT * FROM saln_monitoring_table WHERE year = ? and empNo = ?");
