...
if(isset($_POST['action'])){
    ...
    if($action == 'load'){
        ...
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                echo ' <tr data-bs-toggle="modal" data-bs-target="#exampleModal">
                ...
                <td id="cond">'.$row['smt'].'</td>
                <td id="venue">'.$row['venue'].'</td>
                <td id="expenses">Php '.number_format($row['total'],2).'</td>
                </tr>';
            }
        }
        else{
        ...
        $("table#mainTable tbody tr").on("click",function(){
            ...
            $("li#refList").remove();
            $("tr#exp").remove();
            $("table#mainTable tbody tr:eq("+row+")").each(function(){
