...
// Step 1: Add a click event for the "Update" button/link for each record.

$(".update-button").on("click", function() {
    var recordID = $(this).data("record-id"); // Get the ID from the button's data attribute.

    // Step 2: Send an AJAX request to retrieve the record data based on the recordID.
    $.ajax({
        type: "GET",
        url: "proc_leavecredits.php?id=" + recordID + "&proc=getLeaveData",
        success: function(data) {
            var conv = jQuery.parseJSON(data);
            
            // Step 3: Populate the form fields with the record data.
            $("#vl_bal").val(conv.vl_bal);
            $("#sl_bal").val(conv.sl_bal);
            
            // Additional code to populate other form fields with the record's data.
        }
    });
});
...
$("#update").on("click", function() {
    ...
    // Send an AJAX request with the updated data and the record's ID for updating.
    $.ajax({
        data: {
            id: recordID, // Use the ID of the selected record.
            // Include other updated data fields.
        },
        type: "POST",
        url: "proc_leavecredits.php?proc=update",
        success: function(data) {
            // Display success message or handle any post-update actions.
        },
