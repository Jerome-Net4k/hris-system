...
function loadPersonalInfo($personalInfo){
    ...
    $ipcrQuery = $conn->prepare("SELECT pds.empno, coe_monitoring_table.coe, coe_monitoring_table.datetoday, coe_monitoring_table.dateupload FROM ipcr_encoding_table AS pds INNER JOIN coe_monitoring_table ON pds.empNo = coe_monitoring_table.empNo WHERE coe_monitoring_table.year = $yearSelected AND coe_monitoring_table.empNo = ?"); //PUT THE ORDERED BY HERE
...
// id="viewindividualchecker" value="'. $row['empno'] .'"
            ...
            '<input type="hidden" name="coeUp" value="COE">' .    // change the value hre        
            ...
            <select name="coemonthselect' . $row['empno'] . '" id="coemonthselect' . $row['empno'] . '" class="form-select btn-outline-light" style="margin-left: 10px;" disabled>
            ...
            '<select name="coedayselect' . $row['empno'] . '" id="coedayselect' . $row['empno'] . '" class="form-select btn-outline-light" style="margin-left: 10px;" disabled>
                <script>
                for (var i = 1; i < 32; i++) {
                    var option = document.createElement("option");
                    option.value = i;
                    option.text = i;
                    document.getElementById("coedayselect' . $row['empno'] . '").appendChild(option);
                }
                </script>
            </select>
            </div></div>' 
            ...
            '$("#viewBtn1' . $row['empno'] . '").on("click",function (e){
              ...
              $(document).on(\'change\', \'#uploadFileSelect1' . $row['empno'] . '\', function () {
                            ...
                            document.getElementById("coemonthselect' . $row['empno'] . '").disabled = false;
                            $( "#coemonthselect' . $row['empno'] . '" ).removeClass("btn-outline-light");
                            
                            document.getElementById("coedayselect' . $row['empno'] . '").disabled = false;
                            $( "#coedayselect' . $row['empno'] . '" ).removeClass("btn-outline-light");
             });' . 
                ...
                if(confirm(\'' . $prompt1 . '\')){
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            $( "#coemonthselect' . $row['empno'] . '" ).addClass("btn-outline-light");
                            document.getElementById("coemonthselect' . $row['empno'] . '").disabled = true;

                            $( "#coedayselect' . $row['empno'] . '" ).addClass("btn-outline-light");
                            document.getElementById("coedayselect' . $row['empno'] . '").disabled = true;
...
function searchPersonalInfo($personalInfo){
    ...
    $ipcrQuery = $conn->prepare("SELECT pds.empno, coe_monitoring_table.coe, coe_monitoring_table.datetoday FROM ipcr_encoding_table AS pds INNER JOIN coe_monitoring_table ON pds.empno = coe_monitoring_table.empNo WHERE coe_monitoring_table.year = $yearSelected AND coe_monitoring_table.empNo = ?");
