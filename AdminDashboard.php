...
if (!isset($_SESSION['DatahasbeenFetched'])) {
    header("Location: ../hris-system/index.php");
} else {
    $ShowAlert = true;
    $_SESSION['isUpdated'] = 'false';
}

$sql = "SELECT COUNT(UID), COUNT(program), COUNT(completed) FROM emp_table;";
...
{
    global $conn;
    $sql = "SELECT COUNT($column) FROM emp_table WHERE $column = 1";
    $result = mysqli_query($conn, $sql);
...
{
    global $conn;
    $sql = "SELECT COUNT(UID) FROM emp_table WHERE program = '$columnID'";
    $result = mysqli_query($conn, $sql);
...
{
    global $conn;
    $sql = "SELECT COUNT(UID) FROM `emp_table` WHERE `sex` = 'male'";
    $result = mysqli_query($conn, $sql);
...
{
    global $conn;
    $sql = "SELECT COUNT(UID) FROM `emp_table` WHERE `sex` = 'female'";
    $result = mysqli_query($conn, $sql);
...
{
    global $conn;
    $sql = "SELECT COUNT(UID) FROM emp_table WHERE MONTH(account_Created) = $month";
    $result = mysqli_query($conn, $sql);
            ...
            <div class="container-lg table-responsive-lg">
                <div class="container mt-5 text-bg-light rounded border border-1 border-success"
                    ...
                    <table class="table table-hover align-middle caption-top" id="TraineeTable">
                        ...
                        <tbody>
                            <?php
                            $sql = "SELECT * FROM emp_table WHERE role = 'User' ORDER BY name ASC";
                            $result = mysqli_query($conn, $sql);
            ...
            <div class="container-lg table-responsive-lg">
                <div class="container mt-5 text-bg-light rounded border border-1 border-success"
                    ...
                    <table class="table table-hover align-middle caption-top" id="ProgTable">
                        ...
                        <tbody>
                            <?php
                            $sql = "SELECT DISTINCT title, description, start_date, end_date, start_time, end_time, Duration, hours, progimage, progloc FROM lnd_table ORDER BY title ASC";
                            $result = mysqli_query($conn, $sql);
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    $start = date("h:i A", strtotime($row['lndTo']));
                                    ...
                                    <td class="text-truncate" style="max-width: 100px;" title="' . $row['type'] . '">' . $row['type'] . '</td>
