...
function loadPersonalInfo($personalInfo){
    ...
    $ipcrQuery = $conn->prepare("SELECT pds.empno, nosa_monitoring_table.nosa, nosa_monitoring_table.datetoday, nosa_monitoring_table.dateupload FROM ipcr_encoding_table AS pds INNER JOIN nosa_monitoring_table ON pds.empNo = nosa_monitoring_table.empNo WHERE nosa_monitoring_table.year = $yearSelected AND nosa_monitoring_table.empNo = ?");
...
// id="viewindividualchecker" value="'. $row['empno'] .'"
            ...
            '<input type="hidden" name="nosaUp" value="NOSA">' .    // change the value hre        
            ...
            <select name="nosamonthselect' . $row['empno'] . '" id="nosamonthselect' . $row['empno'] . '" class="form-select btn-outline-light" style="margin-left: 10px;" disabled>
            ...
            '<select name="nosadayselect' . $row['empno'] . '" id="nosadayselect' . $row['empno'] . '" class="form-select btn-outline-light" style="margin-left: 10px;" disabled>
                <script>
                for (var i = 1; i < 32; i++) {
                    var option = document.createElement("option");
                    option.value = i;
                    option.text = i;
                    document.getElementById("nosadayselect' . $row['empno'] . '").appendChild(option);
                }
                </script>
            </select>
            </div></div>' .
            ...
            '$("#viewBtn1' . $row['empno'] . '").on("click",function (e){
              ...
              $(document).on(\'change\', \'#uploadFileSelect1' . $row['empno'] . '\', function () {
                            ...
                            document.getElementById("nosamonthselect' . $row['empno'] . '").disabled = false;
                            $( "#nosamonthselect' . $row['empno'] . '" ).removeClass("btn-outline-light");
                            
                            document.getElementById("nosadayselect' . $row['empno'] . '").disabled = false;
                            $( "#nosadayselect' . $row['empno'] . '" ).removeClass("btn-outline-light");
             });' . 
                ...
                if(confirm(\'' . $prompt1 . '\')){
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            $( "#nosamonthselect' . $row['empno'] . '" ).addClass("btn-outline-light");
                            document.getElementById("nosamonthselect' . $row['empno'] . '").disabled = true;

                            $( "#nosadayselect' . $row['empno'] . '" ).addClass("btn-outline-light");
                            document.getElementById("nosadayselect' . $row['empno'] . '").disabled = true;
...
function searchPersonalInfo($personalInfo){
    ...
    $ipcrQuery = $conn->prepare("SELECT pds.empno, nosa_monitoring_table.nosa, nosa_monitoring_table.datetoday FROM ipcr_encoding_table AS pds INNER JOIN nosa_monitoring_table ON pds.empno = nosa_monitoring_table.empNo WHERE nosa_monitoring_table.year = $yearSelected AND nosa_monitoring_table.empNo = ?");
