...
function loadPersonalInfo($personalInfo){
    ...
    $ipcrQuery = $conn->prepare("SELECT pds.empno, nosi_monitoring_table.nosi, nosi_monitoring_table.datetoday, nosi_monitoring_table.dateupload FROM ipcr_encoding_table AS pds INNER JOIN nosi_monitoring_table ON pds.empNo = nosi_monitoring_table.empNo WHERE nosi_monitoring_table.year = $yearSelected AND nosi_monitoring_table.empNo = ?"); //PUT THE ORDERED BY HERE
...
// id="viewindividualchecker" value="'. $row['empno'] .'"
            ...
            <td class="emp-name" data-value="' . $row['sname'] . '" style="vertical-align: middle;">'.$row['sname'].'</td>
            ...
            '<input type="hidden" name="nosiUp" value="NOSI">' .    // change the value hre        
            ...
            <select name="nosimonthselect' . $row['empno'] . '" id="nosimonthselect' . $row['empno'] . '" class="form-select btn-outline-light" style="margin-left: 10px;" disabled>
            ...
            '<select name="nosidayselect' . $row['empno'] . '" id="nosidayselect' . $row['empno'] . '" class="form-select btn-outline-light" style="margin-left: 10px;" disabled>
                <script>
                for (var i = 1; i < 32; i++) {
                    var option = document.createElement("option");
                    option.value = i;
                    option.text = i;
                    document.getElementById("nosidayselect' . $row['empno'] . '").appendChild(option);
                }
                </script>
            </select>
            </div></div>' 
            ...
            '$("#viewBtn1' . $row['empno'] . '").on("click",function (e){
              ...
              $(document).on(\'change\', \'#uploadFileSelect1' . $row['empno'] . '\', function () {
                            ...
                            document.getElementById("nosimonthselect' . $row['empno'] . '").disabled = false;
                            $( "#nosimonthselect' . $row['empno'] . '" ).removeClass("btn-outline-light");
                            
                            document.getElementById("nosidayselect' . $row['empno'] . '").disabled = false;
                            $( "#nosidayselect' . $row['empno'] . '" ).removeClass("btn-outline-light");
             });' . 
                ...
                if(confirm(\'' . $prompt1 . '\')){
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            $( "#nosimonthselect' . $row['empno'] . '" ).addClass("btn-outline-light");
                            document.getElementById("nosimonthselect' . $row['empno'] . '").disabled = true;

                            $( "#nosidayselect' . $row['empno'] . '" ).addClass("btn-outline-light");
                            document.getElementById("nosidayselect' . $row['empno'] . '").disabled = true;
...
function searchPersonalInfo($personalInfo){
    ...
    $ipcrQuery = $conn->prepare("SELECT pds.empno, nosi_monitoring_table.nosi, nosi_monitoring_table.datetoday FROM ipcr_encoding_table AS pds INNER JOIN nosi_monitoring_table ON pds.empno = nosi_monitoring_table.empNo WHERE nosi_monitoring_table.year = $yearSelected AND nosi_monitoring_table.empNo = ?");
